...
@pytest.mark.parametrize(
    "concurrency",
    [
        {"group": "coverage-main-${{ github.ref }}", "cancel-in-progress": True},
        {"group": "coverage-main-${{ github.ref }}", "cancel-in-progress": "${{ true }}"},
        {"group": "coverage-main-${{ github.ref }}"},
        None,
    ],
)
def test_publisher_never_cancels(documents: Documents, concurrency: object) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        "coverage-main",
        # Keyed on the event as well, an earlier dispatch could finish after a
        # newer push and upload older coverage last.
...
def test_publisher_jobs_declare_no_concurrency(documents: Documents) -> None:
    """A job-level group would govern that job apart from the workflow's."""
    publisher, _ = find_publisher(documents)
    first_job(publisher)["concurrency"] = {"group": "g", "cancel-in-progress": True}
    assert_reports(publisher_violations, documents, "must not declare its own")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("features", "extra", "selection differs"),
    ],
)
